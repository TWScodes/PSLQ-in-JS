...
/**
 * PSLQ Algorithm (Fixed Memory Aliasing)
 */
function pslq(x_in, options = {}) {
    ...
    // --- FIX: Memory Aliasing resolved by slicing properly ---
    const scratch = Array(10).fill(null).map(() => give("0", ["all"], work_prec));
    ...
    let minx = give("Infinity", ["all"], work_prec);
    ...
    for (let i = 0; i < n; i++) {
        ...
        if (temp_a[6](minx)) temp_a[119](minx);
    }
    ...
    for (REP = 0; REP < maxsteps; REP++) {
        ...
        let best_err = give("Infinity", ["all"], work_prec);
        for (let i = 1; i <= n; i++) {
            y[i][114](t_obj); 
        ...
        if (recnorm !== 0n) {
            const recnorm_f = parseFloat(give(recnorm, [], work_prec)[12]());
